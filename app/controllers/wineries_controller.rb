class WineriesController < ApplicationController
  ...
  def search 
    if params[:search].present?
      @wineries = Winery.search(params[:search]).paginate(:page => params[:page], :per_page => 32).order('name ASC')
    else
      @wineries = Winery.all
    end
  end
  ...
  # GET /wineries.json
  def index
    @wineries = Winery.all.paginate(:page => params[:page], :per_page => 32).order('name ASC')
  ...
  def import
    ...
    redirect_to root_url, notice: "Wineries imported."
  end
  ...
  private
    ...
    def winery_params
      params.require(:winery).permit(:name, :country, :region, :sub_region, :green_status, :certification, :website, :about, :thumbnail)
    end
end
